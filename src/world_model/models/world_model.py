...
class MemoryConditionedWorldModel(nn.Module):
    ...
    def __init__(self, image_channels: int = 3, memory_channels: int = 4, hidden_channels: int = 64, pose_dim: int = 12):
        super().__init__()
        self.context_encoder = SimpleConvEncoder(image_channels, hidden_channels)
        ...
        self.fuse = nn.Conv2d(hidden_channels * 2, hidden_channels, kernel_size=1)
    ...
    def encode_context(self, context_rgb: torch.Tensor) -> torch.Tensor:
        ...
        for frame_idx in range(context_rgb.shape[1]):
            encoded = self.context_encoder(context_rgb[:, frame_idx])
    ...
    def forward(
        ...
        for step_idx in range(target_poses.shape[1]):
            ...
            memory_latent = self.memory_encoder(memory_rgbm[:, step_idx])
            fused = self.fuse(torch.cat([memory_latent, pose_map], dim=1))
            hidden = self.temporal_cell(fused, hidden)
            rgb_predictions.append(torch.sigmoid(self.rgb_decoder(hidden)))
